...
def indata():
    ...
    for i in fields:
        #value = input(f'{i} : ')
        value = st.text_input(f'{i} : ')    #streamlit 수정
        data[i] = value
    
    filename = 'used_car_x_test1.csv'
    with open(filename,'w', newline='')  as f:   #저장은 W, 읽어오면 r
        w = csv.DictWriter(f, fieldnames = fields)
        w.writeheader()    #열제목을 파일에 작성
...
def model(filename):
    ...
    st.write('RandomForestRegression:',root_mean_squared_error(y_val,x_predict))
    ...
    df_test = df_test[['year', 'mileage', 'tax', 'mpg', 'engineSize', 'model_A1', 'model_A2',
        ...
        'fuelType_Petrol']]  #train column과 일치시키기

    y_predict = RForest_model.predict(df_test)
    #df_result = pd.DataFrame(df_X_test['id'],columns=['id']) #제목
    #print(y_predict)
